...
Cursor 会从 **`~/.cursor/skills/`**（Windows：`%USERPROFILE%\.cursor\skills\`）**用户级**目录自动发现 Skills，**作用于你打开的所有仓库**（见 [Cursor Agent Skills 文档](https://cursor.com/docs/context/skills)）。**推荐**把本 Skill 整夹放在：

`~/.cursor/skills/claude-code-multi-agent-harness/SKILL.md`

**获取本 Skill（本仓库为独立项目）**：克隆 **[github.com/Jcxu97/claude-code-multi-agent-harness](https://github.com/Jcxu97/claude-code-multi-agent-harness)** 后，将仓库根目录下的 **`SKILL.md`**（及可选 **`README.md`**）放入上述路径；或使用 `git clone` 到临时目录后，将文件夹 **`claude-code-multi-agent-harness`** 复制到 `~/.cursor/skills/`。  
**不**依赖任意业务项目内的 **`alwaysApply` 规则**——默认启用交给 **用户级 Skills 发现**（必要时可在 Cursor **Settings → Rules → User Rules** 用简短文字重申「遵循 harness Skill」）。

## 本版自检（修订时对照）

- **作用域**：默认通过 **`~/.cursor/skills/`（用户级）** 全项目发现；**不**依赖业务仓库内的 **`alwaysApply` 项目规则**（见 **「安装与作用域」**）。  
...
6. **渐进式披露**：`SKILL.md` 保持主干；当前工作区项目若还有专属清单（如易碎路径表），可另建 `references/*.md`，由 Agent **仅在需要时** 读取（对齐 Agent Skills 的 **optional references** 模式）。  
